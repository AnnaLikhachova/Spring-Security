...
import com.userform.google.api.CaptchaSettings;
...
public abstract class AbstractCaptchaService implements CaptchaService {
    ...
    protected void securityCheck(final String response) {
        ...
        if (reCaptchaAttemptService.isBlocked(getClientIP())) {
          new Exception("Client exceeded maximum number of failed attempts");
        }

        if (!responseSanityCheck(response)) {
           new Exception("Response contains invalid characters");
        }
    }
